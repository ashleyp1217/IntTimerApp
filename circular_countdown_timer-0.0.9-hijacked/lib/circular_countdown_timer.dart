...
class CircularCountDownTimerState extends State<CircularCountDownTimer>
  ...
  String _getTime(Duration duration) {
    ...
    // For mm:ss format
      ...
      return '${(duration.inMinutes % 60).toString().padLeft(2, '0')}:${((duration.inSeconds + 1) % 60).toString().padLeft(2, '0')}';
    }
    // For ss format
    else if (widget.textFormat == CountdownTextFormat.SS) {
                ...
                child: Stack(
                  children: <Widget>[
                    ...
                    widget.isTimerTextShown
                        ...
                        : Container(),
                    (widget.warningBeeps && lessThanThree(time))
                        ? warningBeep()
                        : Text(''),
                  ],
                ),
  ...
  List<int> intArr = [];
  bool lessThanThree(time) {
    int i;
    if (widget.isStartCountdown) {
      i = int.parse(time);
    } else {
      List<String> minSec = time.split(':');
      int m = int.parse(minSec[0]);
      if (m > 0) {
        return false;
      }
      i = int.parse(minSec[1]);
    }
    if ((i <= 3) & (i > 0)) {
